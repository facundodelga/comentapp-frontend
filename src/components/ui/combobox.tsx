...
type ComboboxProfileItemProps = ComboboxPrimitive.Item.Props & {
  imageSrc?: string
  imageAlt?: string
  name: string
  description?: string
...
function ComboboxProfileItem({
  ...
  return (
    <ComboboxItem className={cn("min-h-14 py-2 pr-9 pl-2", className)} {...props}>
      <span className="relative flex size-9 shrink-0 items-center justify-center overflow-hidden rounded-full bg-muted text-xs font-semibold text-muted-foreground">
        {imageSrc ? (
          <img
            src={imageSrc}
            alt={imageAlt ?? `Foto de perfil de ${name}`}
